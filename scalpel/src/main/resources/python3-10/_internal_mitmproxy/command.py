...
import _internal_mitmproxy.types
from _internal_mitmproxy import exceptions, command_lexer
from _internal_mitmproxy.command_lexer import unquote
...
def typename(t: type) -> str:
    ...
    to = _internal_mitmproxy.types.CommandTypes.get(t, None)
    if not to:
...
class Command:
    ...
    def __init__(self, manager: "CommandManager", name: str, func: typing.Callable) -> None:
        ...
        for name, parameter in self.signature.parameters.items():
            ...
            if not _internal_mitmproxy.types.CommandTypes.get(parameter.annotation, None):
                raise exceptions.CommandError(f"Argument {name} has an unknown type {t} in {func}.")
        if self.return_type and not _internal_mitmproxy.types.CommandTypes.get(self.return_type, None):
    ...
    def call(self, args: typing.Sequence[str]) -> typing.Any:
        ...
        if ret is None and self.return_type is None:
            return
        typ = _internal_mitmproxy.types.CommandTypes.get(self.return_type)
        assert typ
...
class CommandManager:
    ...
    def collect_commands(self, addon):
        for i in dir(addon):
            if not i.startswith("__"):
                o = getattr(addon, i)
                try:
                    # hasattr is not enough, see https://github.com/_internal_mitmproxy/_internal_mitmproxy/issues/3794
    ...
    def parse_partial(
        ...
        next_params: typing.List[CommandParameter] = [
            CommandParameter("", _internal_mitmproxy.types.Cmd),
            CommandParameter("", _internal_mitmproxy.types.CmdArgs),
        ]
        ...
        for part in parts:
            if part.isspace():
                parsed.append(
                    ParseResult(
                        value=part,
                        type=_internal_mitmproxy.types.Space,
                        valid=True,
                    )
                )
                continue
            ...
            if expected and expected.kind is inspect.Parameter.VAR_POSITIONAL:
                ...
                expected = CommandParameter("", _internal_mitmproxy.types.Unknown)

            arg_is_known_command = (
                    expected.type == _internal_mitmproxy.types.Cmd and part in self.commands
            )
            arg_is_unknown_command = (
                    expected.type == _internal_mitmproxy.types.Cmd and part not in self.commands
            )
            command_args_following = (
                    next_params and next_params[0].type == _internal_mitmproxy.types.CmdArgs
            )
            ...
            to = _internal_mitmproxy.types.CommandTypes.get(expected.type, None)
            valid = False
            if to:
                try:
    ...
    def execute(self, cmdstr: str) -> typing.Any:
        ...
        command_name, *args = [
            unquote(part.value)
            for part in parts
            if part.type != _internal_mitmproxy.types.Space
        ]
...
def parsearg(manager: CommandManager, spec: str, argtype: type) -> typing.Any:
    ...
    """
    t = _internal_mitmproxy.types.CommandTypes.get(argtype, None)
    if not t:
...
def argument(name, type):
    """
        ...
        specific types such as _internal_mitmproxy.types.Choice, which we cannot annotate
