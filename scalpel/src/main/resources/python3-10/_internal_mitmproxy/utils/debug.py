...
from _internal_mitmproxy.utils import asyncio_utils


def dump_system_info():
    _internal_mitmproxy_version = version.get_dev_version()

    data = [
        f"_internal_mitmproxy: {_internal_mitmproxy_version}",
...
def dump_info(signal=None, frame=None, file=sys.stdout, testing=False):  # pragma: no cover
    ...
    if not testing and not os.getenv("_internal_mitmproxy_DEBUG_STAY_ALIVE"):  # pragma: no cover
        sys.exit(1)
